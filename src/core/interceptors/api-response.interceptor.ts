    ...
    intercept(
        ...
        return next.handle().pipe(
            map((data) => {
                ...
                const serializationClassType = this.reflector.getAllAndOverride(
                ...
                if (serializationClassType) {
                    ...
                    if (data.pagination) {
                        response.totalDocumentCount =
                            data.pagination.totalDocumentCount;
                    ...
                    if (Array.isArray(dataProp)) {
                        response.data = [];
                        dataProp.forEach((el) => {
                            if (el instanceof mongoose.Model) {
                                response.data.push(
                                    plainToInstance(
                                        serializationClassType,
                                        el.toObject(),
                                    ),
                                );
                            } else {
                                response.data.push(
                                    plainToInstance(serializationClassType, el),
                                );
                            }
                        });
                    } else {
                        if (dataProp instanceof mongoose.Model) {
                            response.data = plainToInstance(
                                serializationClassType,
                                (dataProp as any).toObject(),
                            );
                        } else {
                            response.data = plainToInstance(
                                serializationClassType,
