...
> {
  ...
  async countImprovementProject() {
    const count = await this.repository.createQueryBuilder('improvement_project')
      .where('improvement_project.status NOT IN (:...status)', {
        status: [ImprovementProjectStatusEnum.DELIVERED],
      })
      .select('COUNT(improvement_project.id)', 'count')
      .getRawOne();
