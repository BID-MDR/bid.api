...
@Injectable()
export class EmployeeRoleBackofficeService extends BaseService<EmployeeRoleEntity, any, any> {
  constructor(
  ...
  async register(data: CreateEmployeeRoleDto): Promise<EmployeeRoleEntity> {
    ...
    if(employee.roles.some(role => role.role === data.role)) {
      throw new BadRequestException("Funcionário já possui essa função.");
    }
