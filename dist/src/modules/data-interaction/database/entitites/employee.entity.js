"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EmployeeEntity = void 0;
const typeorm_1 = require("typeorm");
const base_entity_1 = require("../../../../core/entities/base.entity");
const employee_status_enum_1 = require("../enums/employee-status.enum");
const company_entity_1 = require("./company.entity");
const user_entity_1 = require("./user.entity");
const employee_role_entity_1 = require("./employee-role.entity");
let EmployeeEntity = class EmployeeEntity extends base_entity_1.BaseEntity {
    company;
    user;
    status;
    roles;
};
exports.EmployeeEntity = EmployeeEntity;
__decorate([
    (0, typeorm_1.ManyToOne)(() => company_entity_1.CompanyEntity, company => company.employees),
    __metadata("design:type", company_entity_1.CompanyEntity)
], EmployeeEntity.prototype, "company", void 0);
__decorate([
    (0, typeorm_1.OneToOne)(() => user_entity_1.UserEntity, user => user.employee, { eager: true }),
    (0, typeorm_1.JoinColumn)(),
    __metadata("design:type", user_entity_1.UserEntity)
], EmployeeEntity.prototype, "user", void 0);
__decorate([
    (0, typeorm_1.Column)({
        type: "enum",
        enum: employee_status_enum_1.EmployeeStatusEnum,
        default: employee_status_enum_1.EmployeeStatusEnum.PENDING,
    }),
    __metadata("design:type", String)
], EmployeeEntity.prototype, "status", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => employee_role_entity_1.EmployeeRoleEntity, role => role.employee, { eager: true }),
    __metadata("design:type", Array)
], EmployeeEntity.prototype, "roles", void 0);
exports.EmployeeEntity = EmployeeEntity = __decorate([
    (0, typeorm_1.Entity)({ name: "employee" })
], EmployeeEntity);
//# sourceMappingURL=employee.entity.js.map